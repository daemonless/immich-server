...
RUN if [ -d "packages/plugin-sdk" ]; then \
      ...
      pnpm --filter @immich/sdk --filter immich build; \
    fi
...
# Upstream moved this from /build/corePlugin to /build/plugins/immich-plugin-core
# (a multi-plugin layout) around 2026-07-02; same plugin, just relocated.
COPY --from=linux-immich /build/plugins/immich-plugin-core /app/corePlugin
...
RUN mkdir -p /app/geodata && \
    ...
    fetch -o /app/geodata/countryInfo.txt https://download.geonames.org/export/dump/countryInfo.txt && \
    ...
    fetch -o /app/geodata/ne_10m_admin_0_countries.geojson "https://raw.githubusercontent.com/nvkelso/natural-earth-vector/v5.1.2/geojson/ne_10m_admin_0_countries.geojson" && \
...
ARG PACKAGES="node22 vips ffmpeg p5-Image-ExifTool p5-IO-Compress-Brotli libheif libraw webp alsa-lib sndio sekrit-twc-zimg libopenmpt chromaprint libzvbi fdk-aac libtheora postgresql18-client libva-intel-media-driver mesa-dri libva-utils jq gzip"
ARG UPSTREAM_URL="https://api.github.com/repos/immich-app/immich/releases/latest"
...
# Fix builder umask (COPY --chown preserves restrictive 700/600 from build stage)
...
RUN cp /tmp/immich_version /app/version && \
    ...
    chown -h bsd:bsd /build/www /build/plugins/immich-plugin-core /build/geodata && \
    for v in 14 15 16 17 18; do \
      mkdir -p /usr/lib/postgresql/${v}/bin && \
      ln -sf /usr/local/bin/pg_dump /usr/lib/postgresql/${v}/bin/pg_dump; \
      ln -sf /usr/local/bin/pg_dumpall /usr/lib/postgresql/${v}/bin/pg_dumpall; \
      ln -sf /usr/local/bin/psql /usr/lib/postgresql/${v}/bin/psql; \
    done && \
    chmod -R a+rX /usr/lib/postgresql
